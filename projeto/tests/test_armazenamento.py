...
def test_adicionar_item(mock_supabase):
    mock_response = MagicMock()
    mock_response.data = [
        {"nome": "MORANGO", "quantidade": 100, "limite_minimo": 20, "status": "Ok"}
        ]
    (mock_supabase
    .table.return_value
    .insert.return_value
    ...
    # Verifica se o método de inserção foi chamado com os dados 
    # corretos e cálculo de status
...
def test_atualizar_quantidade(mock_supabase):
    mock_response = MagicMock()
    mock_response.data = [
        {"nome": "AÇAI", "quantidade": 4, "limite_minimo": 5, "status": "Repor"}
        ]
